...
import {ECMFile, isVersionnable} from "../service/storageService";

export namespace DocumentRepository {
    export async function save(eFile: ECMFile):Promise<ECMFile> {
        const createBucketRequest: CreateBucketRequest = {Bucket: eFile.fileLink.bucket};
        // const bucketLocation: GetBucketLocationOutput = await S3.getBucketLocation(createBucketRequest);
        ...
        try {
            ...
            Logger.info('Bucket created and versioning activated:' + JSON.stringify(respVersion));
        } catch (e) {
        ...
        if(!isVersionnable(eFile.docType)){
            throw new Error(`[ERR_DOC_NOVERSION] ${JSON.stringify(eFile)} doesn't allow multiple revision`)
        }
