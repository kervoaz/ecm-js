...
import {DocType, ECMFile} from "../service/storageService";
import {DocumentClient} from "aws-sdk/lib/dynamodb/document_client";


export namespace MetadataRepository {
    ...
    export async function save(eFile: ECMFile) {
        let previousStoredDocument=await getLastRevision(eFile.id);
        ...
        try {
            ...
            const param = {
                ...
                Item: {
                    ...
                    createdAt: documentDTO.createdAt === undefined ? now : documentDTO.createdAt,
            ...
            Logger.error(err);
            throw new Error(`Failed during save documentDTO ${JSON.stringify(documentDTO)} ${err.message}`);
        }
    }
    ...
    export async function list(id: string):Promise<Array<DocumentDTO>> {
        ...
        return <Array<DocumentDTO>><unknown>(await docClient.query(param).promise()).Items;
    }catch(e){
